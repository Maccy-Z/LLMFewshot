...
class MonatoneLogReg(nn.Module, Model):
    ...
    def get_acc(self, xs_target, ys_target):
        ys_target = ys_target.astype(int).squeeze()
        y_pred, y_probs = self.predict(xs_target)
        ...
        ys_target = ys_target.astype(int)
        correct = ys_target == y_pred
    ...
    def plot_net(self):
        ...
        for col, model in enumerate(self.monatone_map):
            with torch.no_grad():
...
def test_logistic_regression():
    ...
    model = MonatoneLogReg(lr=0.02, steps=20)
